...
- [x] **Trade-Items (Gungeon-Sidegrades) — 2026-06-12.** Design-Richtung aus dem Item-Brainstorm: Sidegrades (Trade statt Gain), nie den Skill-Floor anfassen, kein Stat-Stick-Snowball. Drei Items:
  ...
  - **Transmutation Stone** (Shop, 8 Coins): [Q]-Active konvertiert 5 Coins → 1 Key, **oder ab 20 Coins automatisch → 1 Random-Treasure-Item** (auto-upgrade by wallet, Slot-Label zeigt `KEY`/`ITEM`). Zweiter `ActiveItemKind` (`transmuteCoinsToKey`); Details im Active-Item-Block.
...
**Transmutation Stone (zwei Modi, auto-upgrade by wallet — User-Decision 2026-06-12):** [Q] konvertiert **5 Coins → 1 Key** ODER **20 Coins → 1 Random-Treasure-Item**, automatisch: ab `itemCost` (20) Coins feuert die Item-Konversion, darunter die Key-Konversion. Trade-off der Auto-Variante (bewusst akzeptiert): mit vollem Geldbeutel macht [Q] keine Keys mehr — Keys bleiben aber im Shop für 5 kaufbar. **HUD-Mitigation:** der [Q]-Slot-Label zeigt `[Q] KEY` / `[Q] ITEM` je nach Kontostand (via `inventory:changed`), der Threshold-Switch überrascht nie. Item-Roll (`executeTransmuteCoinsToItem`): seeded RNG `${dungeonSeed}-transmute-${rollCount}` (per-run-Counter, SHUTDOWN-reset), Treasure-Pool via `pickItemFromPool` mit **exclude = pickedIds ∪ getFloorReservedItemIds()** (gleiche Union wie `spawnBossPoolItem` — kein Duplikat mit Shop-/Treasure-/Boss-Slots floor-weit). Spawnt als Pedestal 1 Tile über dem Player (normaler Collect-Flow → Toast + SFX + Uniqueness-Bookkeeping). Pool exhausted → graceful fallback auf Key-Konversion. Coins werden nur bei Erfolg gespent (atomic). 20 Coins random ist bewusst ein SCHLECHTERER Deal als ein Shop-Item (8-15, gewählt) — das Premium kauft Verfügbarkeit überall; starke emergente Synergie mit Bloodletter's Pact (+60% Coins) → Economy-Engine-Build. Visual: Gold-Ring am Player (Key = 1 Ring, Item = Doppel-Ring größer+heller) + `playDoorUnlock()`-SFX.
