...
- [~] **Run-Länge-Pass: +50% Räume, Elite-Räume, Minibosse (2026-06-12)** — User-Feedback "Run ~10 Minuten ist recht kurz". Chunk 1+2 DONE, Chunk 3 offen. Siehe Elite/Miniboss-Block unten.
  ...
  - [x] Elite-Räume mit **Champion-System**: pro Floor 1/2/2 Normal-Räume getaggt, dort spawnt EIN elite-promoteter Floor-Mob (HP ×6, Scale ×1.5, Gold-Aura, universeller 6-Thorn-Radial-Burst alle 3s) statt des Packs
  ...
  - [x] Gestaffelte Rewards: Elite-Clear → garantierter Pickup (Heart/Key/Coin-Spray, seeded) + Coin-Burst vom Champion; Miniboss-Clear → Treasure-Pool-Item-Pedestal
...
**Champion-System (`BaseEnemy.promoteToElite(pool)`):** Elite-Raum spawnt EINEN gewichtet gepickten Floor-Mob (Species stable per seed+room) und promotet ihn: HP ×`ELITE_HP_MULT` (6, ON TOP des Floor-Mob-Multiplikators), Scale ×1.5 (Hitbox wächst mit — bewusst, größeres Target), pulsierende Gold-Aura (loses Scene-Object, Cleanup in `die()` UND `destroy()` — Bloomheart-Pattern), und ein **universeller 6-Thorn-Radial-Burst alle 3s** (random Basis-Rotation, feuert auch durch Knockback — sonst wäre ein Champion durch Dauerbeschuss stumm-lockbar). Der Burst ist der Kern-Trick: er macht JEDEN Roster-Pick gefährlich, auch reine Melee-Chaser. Champion-Tod: garantierter 3-Coin-Burst zusätzlich zum normalen Roll. Elite-Raum-Clear: garantierter Pickup (Heart 4 / Key 2.5 / 3-Coin-Spray 3.5 gewichtet, seeded `${dungeonSeed}-elite-reward-${roomId}`).

**Minibosse** (`FloorTheme.minibossIds`, je 1 Kandidat pro Floor; HP authored gegen den Floor-Multiplikator → effektiv ~28/39/54, **kein Boss-DPS-Scaling** — ein gestackter Build darf einen Miniboss shredden, es ist ein Sometimes-Encounter): Reguläre `BaseEnemy`-Subklassen über die normale Factory → leben in der `enemies`-Group, Room-Clear/Türen/Kill-Flow brauchen NULL Sonderbehandlung. Nur Spawn-Entry (`spawnMinibossForRoom`: Raum-Center, vom Player weggeschoben) und Clear-Reward (Treasure-Pedestal via `spawnTreasureItemAt(x, y, desc)` — nutzt den `treasureItemId`-Snapshot + Floor-weite Item-Uniqueness mit) sind eigene Pfade. Uncollected Pedestal überlebt Leave/Re-Entry über die `pendingPickups`-Snapshot-Erweiterung (`captureBossRewards` deckt jetzt `desc.miniboss` mit ab). **Wichtig:** der "leerer Raum → sofort cleared"-Guard in `enterRoom` excludet `desc.miniboss` (sonst Türen sofort auf).
...
Alle drei Texturen prozedural in PreloadScene (64×64, deutlich größere Silhouetten als Mobs, floor-palette-treu). Alle deferred Callbacks mit `!this.active || !this.scene`-Guards (Freeze-Pattern). `coinDropChance: 1` (garantierter Coin on top des Pedestals).
