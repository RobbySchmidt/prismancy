...
- [x] **Trade-Items (Gungeon-Sidegrades) — 2026-06-12.** Design-Richtung aus dem Item-Brainstorm: Sidegrades (Trade statt Gain), nie den Skill-Floor anfassen, kein Stat-Stick-Snowball. Drei Items:
  - **Bloodletter's Pact** (Treasure): Gegner droppen keine Herzen mehr, dafür +60% Coin-Drops. Implementiert als `ItemDefinition.suppressHeartDrops` (DropSystem remappt Heart-Rolls → Coin **post-roll**, RNG-Sequenz + Determinismus unangetastet) + `ItemDefinition.coinDropMult` (multiplikativ aggregiert in ItemSystem, gespiegelt in Registry-Slot `coinDropMultiplier`, gelesen in `BaseEnemy.die` — gleiches Pattern wie `enemyHpMultiplier`). **Boss-Reward-Hearts, Shop-Hearts und Crate-Inhalte bleiben unangetastet** — der Pact verschiebt Heilung in den Shop, er löscht sie nicht. Keine Stat-Effects (`effects: []`).
  ...
  - **Hummingbird Feather** (Treasure): "Flügelschlag-Rhythmus" — +5% fireRate pro Cast in DIESELBE Richtung, Cap +40% (nach 8 Beats), Reset bei Richtungswechsel oder Cast-Pause > `FIRE_RATE_RAMP_BREAK_FACTOR` (2.5) × ungeramptem Intervall. Zwei neue PlayerStats-Felder (`fireRateRampPerCast` / `fireRateRampMax`, base 0); der Beat-State (Stack-Zähler, letzte Richtung, letzter Cast-Zeitpunkt) lebt in `Player.applyFireRateRamp` — analog zum Missile-Modifier-Pattern (Stats = Tunables, Instanz = Mechanik). Funktioniert auf Wizard UND Spellblade (Break-Faktor nutzt das jeweilige Charakter-Intervall). Trade = Achsen-Commitment/Tunnelblick. Erster Cast eines Streaks ist base rate — der Ramp belohnt die Beats NACH dem Commitment.
  - Tests: DropSystem Heart-Remap + RNG-Sequenz-Invarianz (30 Seeds), ItemSystem Drop-Profile-Aggregation (pickUp idempotent per id + hydrate-Replay).
...
- [~] **Run-Länge-Pass: +50% Räume, Elite-Räume, Minibosse (2026-06-12)** — User-Feedback "Run ~10 Minuten ist recht kurz". Chunk 1+2 DONE, Chunk 3 offen. Siehe Elite/Miniboss-Block unten.
  ...
  - [x] Gestaffelte Rewards: Elite-Clear → garantierter Pickup (Heart/Key/Coin-Spray, seeded) + Coin-Burst vom Champion; Miniboss-Clear → **25% Treasure-Pool-Item-Pedestal, sonst Pickup-Paket** (Heart + Key + 3-Coin-Spray; garantiertes Item war "zu viel" — User-Decision 2026-06-12)
...
**Daten-Modell:**
- `ActiveItemKind` discriminator (`'echoesOfBlood'` + `'transmuteCoinsToKey'` seit 2026-06-12). Neue Active = ein Eintrag hier + ein switch-arm in `GameScene.tryActivateActiveItem` + ein `active: { kind: '...' }` Field auf einer ItemDefinition.
