...
- [x] Shop-Logik: 4 Slots (Heart 5 Coins, Key 8 Coins, 2 Items 12-22 Coins — **Price-Pass 2026-06-12 ~+50%**, User: "ich kann die ganze Zeit alles kaufen", weil Run-Länge-Pass + Champion-/Miniboss-Coins die Economy fetter machten; war Heart 3 / Key 5 / Items 8-15), partial-buy via `RoomDescriptor.purchasedShopSlots`, Reject-Wackel bei zu wenig Coins
...
- [x] **Knockback-Resistance (2026-06-12):** neues `EnemyDefinition.knockbackFactor` (default 1; multipliziert die Knockback-Velocity, bei 0 entfällt auch der AI-Lock-Stun). **Possessed Candelabra: 0.25** — der schwere Eisenständer wurde vorher von Dauerbeschuss durch den Raum jongliert. Champions sind via promote-Copy auf 0 gepinnt (komplett immun).
...
*Gem-Siegel (DONE):*
- [x] **`GemSeal`** in `src/entities/GemSeal.ts` — gothic Stone-Altar mit gold Trim + 3 Sockets (one per floor in `REQUIRED_GEM_FLOORS = ['emerald-forest', 'sapphire-swamp', 'onyx-mansion']`). **Pulsierender Attention-Glow (2026-06-12, V2 same day):** zwei Amethyst-Füll-Ellipsen + ein **2px-Stroke-Pulse-Ring** (`0xe8c8ff`, alpha 0.3→0.9 + scale 0.97→1.1) unter dem Altar (Depth FloorDecoration+1), atmen mit 1200ms-Sine-Pulse in versetzter Phase — User-flagged "man checkt überhaupt nicht, dass man damit interagieren kann". **V1 war fills-only bei alpha 0.07–0.3 → auf dem gleich-lila Mansion-Parkett nahezu unsichtbar (User-Screenshot)**; V2 hellere Lavender-Töne, ~verdoppelte Alphas, und der Stroke-Ring als Lesbarkeits-Anker (scharfe Linien überleben auf dunklen Böden, wo weiche Fills absaufen). Fadet bei Activation aus (`fadeOutAttentionGlow`), Cleanup auch im `destroy()`-Teardown-Pfad (lose Scene-Objects, Bloomheart-Pattern). **Sockets starten IMMER empty** (dim + tinted) — die Gems werden live über die `[E]`-Interaktion platziert, nie pre-filled selbst wenn der Spieler mit gefüllter Inventory ankommt. Filled Socket = full color + pulsing halo (per-floor glow color from FLOORS palette). Trigger-Zone größer als Frame damit Player nicht pixel-perfect tappen muss.
...
- [x] **Mob-HP per floor**: `FloorTheme.enemyHpMultiplier` — Emerald **×1.0**, Sapphire **×1.5**, Onyx **×2.0** — **plus gedämpftes DPS-Scaling (2026-06-12)**: `MOB_HP_DPS_SCALING_FACTOR = 0.5`, also `hp × floorMult × (1 + (dpsRatio − 1) × 0.5)`. `BaseEnemy.constructor` reads `enemyHpMultiplier` + `bossHpScale` from the registry. Keeps mob threat in line with player damage growth across floors AND builds (User: "jeden Run absolut OP").
- [x] **Boss-DPS-Scaling**: every boss instantiation calls `GameScene.updateBossHpScale()` first, computes `(currentDamage × currentFireRate) / (BASE_DAMAGE × BASE_FIRE_RATE)`, clamps ≥ 1.0, stores on registry. **`BossEnemy.constructor`** + **`VampireBody.constructor`** read it and override BaseEnemy's mob-mult-applied `this.hp` with `Math.round(definition.hp × scale × bossFloorMult)` (also sets `this.maxHp`). `bossFloorMult` = `FloorTheme.bossHpMultiplier` (×1.0/×1.25/×1.5 — 2026-06-12 bump, weil die Ratio Multishot/Burst/Pierce/Burn nicht erfasst und Late-Run-Bosse relativ zum Miniboss melteten). Result: bosses always feel like at-least-base-stats fights regardless of build — Phase 1/2/3 special timers always fire because phase HP-blocks scale linearly with player DPS.
...
- [x] **Trade-Items (Gungeon-Sidegrades) — 2026-06-12.** Design-Richtung aus dem Item-Brainstorm: Sidegrades (Trade statt Gain), nie den Skill-Floor anfassen, kein Stat-Stick-Snowball. Drei Items:
  - **Bloodletter's Pact** (Treasure) — **V2-Rework 2026-06-12, jetzt [Q]-Active**: V1 war ein passiver Economy-Trade (keine Herzen, +60% Coin-Drops via `suppressHeartDrops` + `coinDropMult`, später scope-erweitert auf alle Heart-Quellen außer Shop) — User-Playtest-Verdikt: **"das Item zerstört die komplette Money-Economy"**. Neuer Effekt: **[Q] opfert einen Herz-Container (max HP −2) für permanent +20% auf einen seeded-random Core-Stat** (damage / fireRate / moveSpeed — **`missileSpeed` aus dem Pool entfernt 2026-06-14**, weil ein Projectile-Speed-Roll als Gegenwert für einen Herz-Container frustrierend war). Wiederholbar bis nur noch 1 Container übrig ist (`BLOOD_PACT_HEART_COST + 2`-Gate in `tryActivateActiveItem`, gespiegelt im ActiveItemSlot via `currentMaxHp >= 4`). `ActiveItemKind.SacrificialStatUp` + `ActiveItemSpec.statUpMult` (1.2). Roll seeded `${dungeonSeed}-bloodpact-${useCount}`. **Stat-Ups leben in `ItemSystem.sacrificialStatUps`** (runtime-granted, nicht item-bound — können NICHT aus `pickedItemIds` replayed werden) und carryen über Floor-Wechsel via neuem `RunCarryOver.sacrificialStatUps`-Field; der HP-Cost carried automatisch über `healthMax`. Neue API: `PlayerHealth.removeMaxHealth(amount)` (floor bei 2 HP, kann nie killen), `ItemSystem.grantSacrificialStatUp(stat, mult)`. Feedback: Crimson-Ring + floatendes "+20% DAMAGE"-Label + Red-Flash + playPlayerHit. **Die `suppressHeartDrops`/`coinDropMult`-Infrastruktur (DropSystem-Remap + zentraler `spawnPickup`-Remap + Tests) bleibt im Code, ist aber DORMANT** — kein Item nutzt sie mehr. **V2 user-validated** ("fühlt sich super an").
  - **Transmutation Stone** (Shop, 12 Coins — war 8): [Q]-Active konvertiert 5 Coins → 1 Key, **oder ab 30 Coins automatisch → 1 Random-Treasure-Item** (auto-upgrade by wallet, Slot-Label zeigt `KEY`/`ITEM`; `itemCost` 20 → 30 mit dem Shop-Price-Pass 2026-06-12, damit der Random-Roll ein Premium über dem 12-22er Shop-Item bleibt). Zweiter `ActiveItemKind` (`transmuteCoinsToKey`); Details im Active-Item-Block.
  - **Hummingbird Feather** (Treasure) — **V2-Rework 2026-06-12, jetzt Triple-Burst**: V1 war der "Flügelschlag-Rhythmus"-Fire-Rate-Ramp (+5%/Cast in dieselbe Richtung, Cap +40%) — User wollte stattdessen Bursts. Neuer Effekt: **jeder Cast-Trigger feuert eine 3-Schuss-Salve** (`burstCount`-Stat, base 1, Feather addiert +2). Schuss 1 sofort, Rest im `BURST_SHOT_GAP_MS = 80`-Takt, Richtung beim Trigger gelockt (kein Mid-Burst-Re-Aim — die Salve committet). Cooldown ZWISCHEN Salven = `BURST_COOLDOWN_FACTOR = 2.4` × Basis-Intervall → Wizard: 3 Schuss / 600ms statt 1 / 250ms ≈ **+25% sustained DPS** mit front-loaded Burst-Damage — Rhythmus-Item, kein flaches ×3. Implementation: `Player.handleAttacking` owned das Burst-Scheduling (delayedCalls mit `!active || !isAlive`-Guard), `fireWizardMissile`/`fireSpellbladeBolt` sind auf reine Single-Shot-Spawner reduziert (`castShot(dir)`-Dispatch, Cooldown-Bookkeeping lebt nur noch in handleAttacking). **Ramp-Code komplett entfernt**: `fireRateRampPerCast`/`fireRateRampMax` Stats, `applyFireRateRamp`, `FIRE_RATE_RAMP_BREAK_FACTOR`. Funktioniert auf Wizard UND Spellblade; Stats werden pro Burst-Schuss frisch gelesen (Multishot × Burst = 2×3 Bolts stackt). **V2 user-validated** ("fühlt sich super an").
  - Tests: DropSystem Heart-Remap + RNG-Sequenz-Invarianz (30 Seeds), ItemSystem Drop-Profile-Aggregation (pickUp idempotent per id + hydrate-Replay) — decken die (jetzt dormante) suppressHeartDrops-Infra weiter ab.
  - **Zurückgestellt, nicht verworfen:** Wand-Schuss-Stile als austauschbare Kategorie (Ricochet/Boomerang/Sapper/Wave, latest-wins wie Gungeon-Guns) — stärkster struktureller Vorschlag aus dem Brainstorm, wartet auf User-Entscheidung nach dem Playtest der drei Trade-Items.
- [~] **Run-Länge-Pass: +50% Räume, Elite-Räume, Minibosse (2026-06-12)** — User-Feedback "Run ~10 Minuten ist recht kurz". Chunk 1+2 DONE, Chunk 3 offen. Siehe Elite/Miniboss-Block unten.
  - [x] Progressive Raumanzahl: `FloorTheme.roomCount` 10/12/14 (Emerald/Sapphire/Onyx, war flat 8)
  - [x] Elite-Räume mit **Champion-System**: pro Floor 1/2/2 Normal-Räume getaggt, dort spawnt EIN elite-promoteter Floor-Mob (HP ×6, Scale ×1.5, **rote Aura**, Dual-Wave-6-Thorn-Radial alle 2.2s, Move-Speed ×1.3 — Aggression-Pass 2026-06-12 weil Champion immer solo spawnt) statt des Packs
  - [x] **Minibosse** (35% Chance pro Floor, max. 1 Raum): drei neue dedizierte Gegner — Thornwood Shambler (Emerald), Mire Lurker (Sapphire), **Doppelgänger** (Onyx; ersetzte den ursprünglichen Headless Knight noch am selben Tag, inkl. Difficulty-Pass — User-validated "fühlt sich sau geil an")
  - [x] Gestaffelte Rewards: Elite-Clear → **nur noch Champion-eigene Drops** (garantierter Coin + 3-Coin-Death-Burst; der zusätzliche Raum-Clear-Pickup war Double-Dipping — User-Playtest 2026-06-12, `spawnEliteClearReward` entfernt); Miniboss-Clear → **25% Treasure-Pool-Item-Pedestal, sonst Pickup-Paket** (Heart + Key + 3-Coin-Spray; garantiertes Item war "zu viel" — User-Decision 2026-06-12)
  - [x] **Miniboss-HP-Leiste**: kleine silberne Top-Bar (200px, unter der Boss-Bar-Position) mit Namen — klar unterscheidbar von der 400px-Gold-Boss-Bar. Eigene `miniboss:*`-Event-Familie, emittiert aus BaseEnemy via `EnemyDefinition.miniboss`-Tag
  - [ ] **Chunk 3 (offen):** neue dedizierte Elite-Gegner zusätzlich zu den Champion-Varianten (User wollte "beides")
...
- [x] **Active Items mit [Q]** — Scope-Pivot 2026-05-09. War vorher out-of-scope (Entscheidung 2026-05-07), jetzt reaktiviert mit minimaler Infrastruktur (`ItemDefinition.active`, `ActiveItemSystem`-Slot-Owner, `ActiveItemSlot`-HUD-Widget bottom-left, [Q]-Key-Polling in `tickActiveItem`). Erstes Active-Item: **Blood of Marquis** — Glass-Cannon-Trade, +30% all stats permanent, max HP locked auf 2 Hearts, [Q] feuert "Echoes of Blood" AOE die Trash-Mobs instant-killt + Bossen 30% max-HP-Damage zufügt (Phase-Threshold cross natürlich). Drop-Gate via `ItemDefinition.metaUnlock = 'boss-marquis-of-mirages'`. **Pool-Move 2026-05-09:** ursprünglich im Onyx-Boss-Pool platziert, war damit aber praktisch sinnfrei — User-Logik: "man muss den Marquis erstmal schlagen um's freizuschalten, dann hat man im selben Run nur noch den Prismarch übrig zum Einsetzen, und um's überhaupt erstmal zu sehen muss man den Marquis ein zweites Mal across runs schlagen". Ins **Treasure-Pool** verschoben (floor-agnostic), `floor`-Tag entfernt. Ab dem zweiten Run pro-Marquis-Kill kann's auf jedem Floor in einem Treasure-Raum spawnen — ein mid-run pivot pickup statt eines Late-game-Trophy-Items. Siehe Active-Item-System-Block unten.
...
- **Wizard Glasses (Multishot +1 → 2 parallele Bolts à 80% damage)** — feuert pro Cast zwei Spellblade-Bolts mit 14 px perpendikulärem Gap. Beide Bolts erben dieselbe `bodyRadiusOverride`, `inheritVx/Vy` und `rotateToDirection` Settings → identische Flight-Physik, parallel. Pierce-Baseline +1 gilt pro Bolt (jeder Bolt pierced 1 Enemy zusätzlich). Wenn Spellblade an `body.right` Wall-Edge gepresst ist und nach Süden castet, sitzt die Pair am east-west-Offset → der west-Bolt steht innerhalb der Wall-Hitbox. `bodyRadiusOverride` (= `MISSILE_RADIUS`) verhindert dass der west-Bolt die Wall überlappt. Visual mit 1.5×-Spellblade-Visual-Scale: zwei rotierte Diamond-Sprites parallel.
...
**Floor-HP-Scaling (Anti-Melt — replaces damage cap):** Drei Tiers, alle registry-driven so der Schaden lokal in `BaseEnemy.constructor` / `BossEnemy.constructor` bleibt. **2026-06-12 Balance-Pass** (User: "ich bin nun eigentlich jeden Run absolut OP" + "Doppelgänger-Fight dauerte länger als Marquis + Prismarch zusammen"):

*Mobs (per-floor static × gedämpftes DPS-Scaling):* `FloorTheme.enemyHpMultiplier` — Emerald **×1.0**, Sapphire **×1.5**, Onyx **×2.0** — PLUS gedämpfter DPS-Anteil: `hp = round(definition.hp × floorMult × (1 + (dpsRatio − 1) × MOB_HP_DPS_SCALING_FACTOR))` mit Faktor **0.5**. Bei 3× Base-DPS hat ein Mob ×2 HP statt ×3 (voll) oder ×1 (keins) — gestackte Builds clearen sichtbar schneller als base (Power-Fantasy bleibt), aber Trash verdampft nicht mehr. Bei Base-Stats kollabiert die Formel auf den reinen Floor-Mult (frische Runs unverändert). `spawnEnemiesForRoom` + `spawnEnemyAt` rufen `updateBossHpScale()` vor jedem Spawn-Batch damit der Registry-Wert den aktuellen Build reflektiert.

*Minibosses (floor mult × VOLLES DPS-Scaling):* `hp = round(definition.hp × floorMult × dpsRatio)` — build-invariante Time-to-Kill wie bei Bossen (Reversal 2026-06-12, siehe Miniboss-Block).

*Bosses (dynamic DPS-ratio × per-floor Bump):* Vor jedem Boss-Spawn ruft `GameScene.updateBossHpScale()` → berechnet `(currentDamage × currentFireRate) / (BASE_DAMAGE × BASE_FIRE_RATE)`, clamp ≥ 1.0, schreibt nach `registry.set('bossHpScale', …)`. **`BossEnemy.constructor`** liest den Wert nach dem `super()`-Call und überschreibt BaseEnemy's hp mit `round(definition.hp × scale × bossFloorMult)`. **`FloorTheme.bossHpMultiplier`** (Emerald ×1.0 / Sapphire **×1.25** / Onyx **×1.5**, gespiegelt via Registry-Slot `bossFloorHpMultiplier` in `GameScene.init`) ist der 2026-06-12-Bump: die DPS-Ratio erfasst nur damage × fireRate — Multishot/Burst/Pierce/Burn NICHT — wodurch Late-Run-Builds die Floor-2/3-Bosse relativ zum voll skalierten Miniboss gemeltet haben. **Effekt:** Bosse fühlen sich immer mindestens wie ein Base-Stats-Fight an; ab Floor 2 etwas drüber. Phase-1/2/3-Special-Timer (P1=6s, P2=5s, P3=4s) feuern weiterhin garantiert.
...
- **`DepthLayers.Player = 75`** (war 50, jetzt über `Wall = 70`). Folgekorrektur zur +12-Body-Verschiebung: ohne den Depth-Swap clipped der Wizard-Hut visuell in die Top-Wall-Pixel rein (Body stoppt 1 px innerhalb der Wand-Tile, aber Hut ragt ~32 px über den Body). User-Wunsch war "z-index vom wizard über die Wand legen" statt die Wand-Collider zu extenden — gameplay-Hitbox bleibt genau gleich, nur die Render-Reihenfolge ändert sich.
- **`DepthLayers.Enemy = 72`** (war 40, **gebumped 2026-05-09**, jetzt über `Wall = 70` und unter `Player = 75`). Selbe Logik wie beim Player: bei den 2.4×-skalierten Sapphire/Onyx-Bossen (Bog Colossus, Bloomheart, Marquis of Mirages, Lord Onyx) ragten die Sprites visuell weit über ihren Physics-Body hinaus — bei Boss-Position nahe der Top-Wall wurde der Sprite-Top abgeschnitten weil die Wall-Texture obenauf gezeichnet wurde. Mit Depth=72 rendern Enemies + Bosse über der Wand-Texture. Hitboxen + Wall-Collision unverändert.
- Rocks + Trees: **keine Hitbox**, reine Boden-Deko auf `DepthLayers.FloorDecoration` (unter Spieler/Gegner) — User-Feedback: Steine machten als Hindernis im Bewegungsfluss keinen Sinn, Bäume verbergen Pixies hinter ihrer Krone, und alles soll unter Spieler/Gegnern sein. `Room.treePositions` trackt die Tree-Positionen weiter, damit Pixie Queen zwischen Bäumen teleportieren kann.

**Wizard ↔ Spellblade Hitbox-Parität (2026-05-09 Investigation):** User-Bug "ich werde irgendwie immer ganz komisch getroffen mit spellblade. gefühlt hängt die hitbox nach unten und links". Code-Side sind die Hitboxen identisch — beide Charaktere durchlaufen denselben `Player.constructor` mit demselben `setCircle(PLAYER_HITBOX_RADIUS / WORLD_SPRITE_SCALE, halfW - radius, halfH + PLAYER_HITBOX_OFFSET_Y/scale - radius)` ([Player.ts:158](src/entities/Player.ts#L158)), beide Texturen sind 64×64, beide auf WORLD_SPRITE_SCALE=1.25. Body-Center bei (sprite.x, sprite.y + 10), world-radius 11. Mathematisch zero-difference. Triage via temporäres F2-Debug-Overlay (Phaser's `world.createDebugGraphic()`) bestätigt: identische Position. Die "down-left"-Wahrnehmung kommt aus drei Visual-Faktoren:
1. **Spellblade-Cape extends asymmetrisch** in PreloadScene `drawSpellbladeTexture` (line 704) — Hem-Polyline hat `cx-13` / `cx+13` extents + windblown bulges (cx-10 / cx+11) + 4 Hem-Streamers über die Cape-Hem-Linie hinaus. Visual mass extends down-left vom echten Body. Cape ist intentional asymmetrisch ("windblown rather than stiff sheet") — bleibt erhalten.
2. **Spellblade hat Ground-Shadow** bei `g.fillEllipse(cx, figBot=53, 14, 2)` (PreloadScene line 921) — visueller Boden des Sprites ist bei frame y=53. Wizard hatte **keinen** Ground-Shadow → visueller Boden bei den Boots (frame y=44). Same hitbox (frame y=40 center, span 31..49) liest beim Wizard als "tief am Boden" (4 px über Boot-Linie), beim Spellblade als "hoch im Body schwebend" (13 px über Shadow-Linie). **Fix:** Wizard kriegt jetzt auch einen Ground-Shadow am gleichen `figBot=53`-Spot mit identischen Dimensions/Alpha (`g.fillEllipse(cx, figBot, 14, 2)` mit `palette.SHADOW, 0.45` in [PreloadScene.ts:386](src/scenes/PreloadScene.ts#L386)). User-bestätigt: "es fühlt sich definitiv so an wie beim wizard jetzt".
3. **Wizard-Belt verdeckt obere Circle-Hälfte im Debug-Overlay** — Belt ist Gold (BUCKLE color) bei frame y=40-43, Hitbox-Center ebenfalls bei y=40. Phasers Debug-Graphic zeichnet die Body-Outline grün, Gold + Grün overlap macht den Wizard-Hitbox-Kreis schwerer sichtbar als den Spellblade-Kreis (der über der dunklen Breastplate liegt). Hat keine Gameplay-Bedeutung, nur Wahrnehmungs-Artefakt beim Vergleichen via Debug-Overlay.

Untersucht via temporärem F2-Toggle in GameScene; das Overlay wurde nach Verifikation wieder entfernt.
...
**Bloomheart-Spore-Cleanup-on-Death (Freeze-Fix 2026-06-06):** Die Phase-2-Spore ist ein **loses** `scene.add.circle` (in keiner Group), dessen Drift-Tween-`onComplete` einen 6-Thorn-Burst feuert + via `this.scene.add.circle(...)` ein Pop-Ring-Visual erzeugt. `BLOOMHEART_SPORE_LIFETIME_MS = 700` ms, aber der `BaseEnemy.die()`-Death-Tween läuft nur 220 ms bis `this.destroy()`. **Jede Spore, die in den ~480 ms vor dem Boss-Tod startet, beendet ihren Tween erst NACH dem `destroy()`** → `onComplete` läuft mit zerstörtem Sprite (`this.scene === undefined`) → Exception **im Phaser-Update-Loop** → Game freezed (Render-Loop bricht ab). User-Bug "freezed manchmal nach dem Bloomheart-Fight"; **Burn-DoT (Fire Orb) erhöhte die Häufigkeit** weil der Tick den Boss off-beat zwischen den Fan-Zyklen killt (höhere Chance dass eine Spore fliegt). Fix: Bloomheart trackt live Sporen in `activeSpores` (spore + halo + tween) und ruft `clearSpores()` (stoppt Tweens → kein `onComplete` mehr + zerstört Objekte) aus **`die()` UND `destroy()`** (immediate-on-death + teardown-Pfad). Plus defensiver `if (!this.active || !this.scene)`-Guard im `onComplete` als Belt-and-Suspenders. **Generelles Muster:** ein deferred Callback (Tween-`onComplete` / `delayedCall`), der nach Boss-Tod auf `this.scene` oder lose Objekte zugreift, ist ein Freeze-Risiko — wer einen neuen Boss mit losen getweenten Objekten baut, muss sie beim Tod aufräumen.
...
**Reposition-Safe-Spot mit Heading-Cone (Vine Lord + Bloomheart, 2026-06-13):** Phase-2-Repositionsbosse, die untertauchen + woanders wieder auftauchen und SOFORT einen radialen Burst feuern, dürfen nie im Laufweg des Spielers auftauchen — sonst steckt er zwischen Boss und Burst (User-flagged unfair). `pickReemergeSpot()` filtert jetzt zweistufig: (1) Hard-Min-Distance (`*_MIN_PLAYER_DIST = 3*64`, wie gehabt), (2) **NEU:** wenn der Spieler sich bewegt (Body-Velocity ≥ `*_PLAYER_MOVING_EPS = 30` px/s), werden alle Kandidaten im ±`*_HEADING_CONE_RAD`-Kegel (= 60°, also 120° total) VOR der Laufrichtung verworfen (Dot-Product `candDir·velDir > cos(cone)` → ausgeschlossen). Fallback-Kette: „weit genug UND nicht im Laufweg" → „nur weit genug" → weitester Kandidat. Steht der Spieler still, gibt's keinen Laufweg → nur Min-Distance zählt. Implementiert für **Vine Lord** (`VINE_LORD_BURROW_HEADING_CONE_RAD` / `VINE_LORD_BURROW_PLAYER_MOVING_EPS`) Phase-2-Burrow + **Bloomheart** (`BLOOMHEART_SINK_HEADING_CONE_RAD` / `BLOOMHEART_SINK_PLAYER_MOVING_EPS`) Phase-2-Sink-&-Re-Bloom. Verwandt mit dem Pixie-Queen-Teleport-Safe-Distance oben — wer einen neuen Reposition-Boss baut, sollte denselben Heading-Cone-Filter mitnehmen.
...
**Floor-Themed Default-Projektil-Texture (2026-05-09, erweitert 2026-06-12):** `EnemyProjectilePool` hat jetzt einen `defaultTextureKey`-Field der per Floor gesetzt wird. GameScene.create setzt direkt nach Pool-Construction: Sapphire → `SapphireThorn`, **Onyx → `BloodProjectile`** (rund/rot — User-flagged: der pfeilartige Emerald-Thorn passte beim Onyx-Champion-Burst nicht zum Mansion-Theme; alle regulären Onyx-Mobs/-Bosse übergeben ihre Texture per-call, der Default surfact dort nur über generische Firer wie den Elite-Burst), sonst `Thorn`. Ein neuer `TextureKeys.SapphireThorn` (gezeichnet in `PreloadScene.drawSapphireThornTexture`, identische 36×22-Silhouette wie Thorn aber blau/cyan-codierte 4-Tone-Palette: deep-sapphire base 0x041a30 → mid-blue 0x2d5a99 → cyan ridge 0x88dcff → cyan-glow halo 0x4ad8ff) ersetzt den default Thorn auf dem Sapphire-Floor. Affected: alle 4 Sapphire-Bosse (Toad Sovereign, Bloomheart, Damselfly Empress, Bog Colossus) + alle 4 Sapphire-Mobs (Bog Frog, Snapper Bloom, Damselfly, Bog Tortoise) — keiner musste angefasst werden, der Pool-Default liefert die richtige Texture automatisch. Bosse mit eigenen Bullet-Texturen (Marquis BloodProjectile, Mirror MansionMissile, Candelabra FlameMissile, LordOnyx-Specials) übergeben weiterhin per-call `textureKey` und bypassen den Default. Emerald + Onyx Floor: defaultTextureKey bleibt `Thorn` — Onyx-Bosse/Mobs nutzen sowieso ihre eigenen non-default-Texturen.

**Pixie-Queen pinke Orbs (2026-06-13):** Die Pixie Queen feuert pinke RUNDE Projektile statt der Default-Pfeil-Thorns (User: runde Feen-Magie passt thematisch besser). `TextureKeys.PixieOrb` (16×16, via die parametrisierte `drawCasterOrbTexture(g, key, halo, mid, core)` mit Pink-Palette `0xff8ad0`/`0xff4f9e`/`0xffd6ec` erzeugt — kein eigener Zeichen-Code) wird in BEIDEN `PixieQueen`-Feuer-Pfaden (Shot-Fan + Teleport-Landing-Burst) als per-call `textureKey` an `enemyProjectilePool.fire(...)` übergeben. 16×16 = gleiche Maße wie BloodProjectile/CasterOrb → der EnemyProjectile-Hitbox-Recenter greift automatisch (kein Hitbox-Drift).

**Boss-Projektil-Range-Bump (2026-05-09):** `ENEMY_PROJECTILE_LIFETIME_MS` von 1500 → **2200 ms** in `GameConfig.ts`. User-Bug: rooted middle-room-Bosse (Forest Heart, Bloomheart) konnten gecheesed werden indem der Spieler an die Far-Wall lief — Player-Missile-Range (378 px) reichte für den Hit, Boss-Projektil-Range (240 × 1.5 = 360 px) reichte nicht zurück. Bei 240 px/s ist 2200 ms = **528 px Range**, deckt vom Raum-Mittelpunkt (480, 288) jede Player-Position bis zur Ecke (~472 px max) mit Buffer. Bosse mit höherer Speed (Toad-Tongue + Damselfly bei 280 px/s) bekommen 616 px. Bosse mit eigenen `setLifetime()`-Overrides bleiben unverändert: alle Onyx-Patterns (Forest Wrath, Tide Mandala, Crimson Web), BogColossus-Orbit, CursedMirror-Homing, MarquisOfMirages-Homing. Side-Effect: stationäre Mobs (Vine Sprout cardinal-Shooter, Snapper Bloom 3-Thorn-Fan) reichen jetzt auch 528 px statt 360 — bewusst, gibt ihnen positional weight ohne Bullet-Hell weil sie weiterhin solo schießen.
...
**In-Game Dev-Affordances (TEMPORARY — vor Production-Release entfernen):** Zwei In-Scene-Hotkeys in GameScene um Hard-Mode-Testing zu beschleunigen, beide markiert mit `**TEMPORARY**` JSDoc damit ein Grep nach dem Marker beim Cleanup alle Stellen findet.

- **[F2] Hitbox-Debug-Overlay** — togglet Phaser's `physics.world.drawDebug`. Erste Toggle creator das DebugGraphic via `createDebugGraphic()`, alle weiteren togglen Visibility. Depth auf 2000 gepinnt damit's über HUD + Map-Mode liegt. Zeigt Player-Hitbox-Kreis, Enemy/Boss-Bodies, Wall-Strip-Colliders, Door-Barriers, Enemy-Projectile-Bodies. Cleanup-Surface: ein `keydown-F2`-Handler in `create()`.

- **[B] Boss-Spawn-Picker** — In-Scene Container-Overlay (kein separates Scene-Registration nötig) das alle 10 Bosse in 3 floor-gruppierten Sections listet (Emerald Forest grün, Sapphire Swamp cyan, Onyx Mansion purple). Click auf einen Row → `closeBossSpawnMenu()` + `devSpawnBoss(id)` (gleicher Code-Pfad wie `__wiz.spawnBoss(...)`). Hover-Effekt + Hand-Cursor auf den Rows. **[B]** oder **[ESC]** schließt. ESC-Handling im `update()`-Poll (nicht als Event-Listener) damit's mit dem Pause-Menu nicht racet — `keydown-ESC` events feuern synchron VOR `update()`, was zu "Boss-Menu close → Pause-Menu open" Doppel-Trigger im selben Frame führen würde. Cleanup-Surface: `bossSpawnMenu` Field, `setupBossSpawnMenu` + `openBossSpawnMenu` + `closeBossSpawnMenu` Methoden, der `setupBossSpawnMenu()` Aufruf in `create()`, das `this.bossSpawnMenu = null` im SHUTDOWN-Block, und der `this.bossSpawnMenu`-Branch im `update()`-ESC-Check.
...
**Wall-Collider-Refactor (2026-05-09):** Der Wand-Stuck-Snag entlang der vertikalen Wände (links + rechts) wurde gefixt indem die Wände komplett umgebaut wurden — von **per-Tile static bodies** auf **vier durchgehende lange Static-Bodies** (einer pro Wand-Seite, ggf. an der Door-Tile-Position in zwei Halb-Strips gesplittet). Visuelle Wall-Tile-Sprites bleiben einzeln (für Texture-Rendering), aber haben keinen eigenen Body mehr. Die Strip-Bodies sind invisible `Phaser.GameObjects.Rectangle`s mit `physics.add.existing(rect, true)` + explizitem `body.setSize(w, h) + body.updateFromGameObject()` damit die Body-Größe garantiert mit der Rectangle übereinstimmt (Phasers Default-Body-Derivation kann bei Texture-loser Rectangle off sein). Gespeichert in der existierenden `walls`-StaticGroup, alle existing Collider-Registrierungen (player/enemy/missile/enemy-projectile vs walls) funktionieren unverändert. Corner-Overlap zwischen Top/Bottom + Left/Right vermieden indem Left/Right-Strips nur ty=1..ROOM_HEIGHT-2 abdecken (Eck-Tiles gehören zu Top/Bottom). Visuelle Tile-Sprites werden im neuen `wallVisuals: GameObject[]` Field tracked + im `destroy()` aufgeräumt. Result: keine internen Tile-Nähte mehr in der Wand, Phasers circle-vs-AABB-Separator kann an Mid-Wall-Tiles nicht mehr die falsche Achse picken — User-bestätigt für Mid-Wall-Slides.

**Door-Edge-Snag (2026-05-09 — TEILWEISE behoben, leicht spürbar):** Die Tür-Edges blieben nach dem Wand-Refactor noch ein Hänger weil an der Stelle wo Wall-Strip endet und Door-Barrier beginnt eine **exact-aligned Tile-Edge** zwischen zwei Static-Bodies sitzt — Phaser picked an dieser Naht bei rect-vs-rect die wrong-axis-Separation. Zwei zusätzliche Fix-Schichten:

1. **Disabled End-Faces auf Split-Strips** in `Room.addHorizontalWallStrip / addVerticalWallStrip`: Die Door-zugewandte Face jedes Half-Strips wird via `body.checkCollision` deaktiviert. Top/Bottom-Wand → East-Face (linker Strip) / West-Face (rechter Strip). Left/Right-Wand → South-Face (oberer Strip) / North-Face (unterer Strip). Theorie: der Separator kann nicht mehr perpendikulär zur Slide-Richtung pushen, nur entlang der Wand-Achse. Defensive Begründung im Code-Kommentar weil die Logik subtil ist.

2. **Door-Barrier-Body extends 4 px in adjacent Strips** in `Door.close()`: Statt 64×64 ist die Barrier-Body jetzt 72×64 (horizontale Türen) oder 64×72 (vertikale Türen). Die 4 px ragen in beide Wall-Strips rein — `BARRIER_OVERLAP = 4`. Texture bleibt 64×64 am Tile-Center (visual unverändert). Theorie: an der vorherigen exact-aligned-Naht zwischen Strip + Barrier wurde der Player auf isolated-edge-Position erwischt, mit 4-px-Overlap ist er IMMER innerhalb mindestens einer Body-Region und der Übergang ist seamless. Cross-Axis-Faces der Barrier bleiben weiter disabled (`up/down` für vertikale Türen, `left/right` für horizontale).

**Status:** Der Snag ist von "stuck completely" → "leichter Hänger, kann weiterlaufen" reduziert, aber nicht 100% weg. User-Feedback "ist immernoch ein leichter hänger". Mögliche restliche Ursachen: Floating-point precision (body.top minimal != 64), Phaser's TILE_BIAS = 16 amplifiziert tiny overlaps, oder fundamentales circle-vs-AABB-corner-rolling das ohne Switch zu Rectangle-Body / Matter.js nicht 100% eliminierbar ist. **Untried Approaches** für einen späteren Polish-Pass dokumentiert in `next_session.md`: Player-Body von Circle auf Rectangle wechseln, TILE_BIAS reduzieren, größerer Overlap (8-12 px statt 4), per-frame slip-detection in Player.preUpdate.
...
**EnemyProjectile-Hitbox-Recenter (2026-06-06):** `EnemyProjectile.fire()` ruft jetzt **nach** dem `setTexture` ein erneutes `setCircle(ENEMY_PROJECTILE_RADIUS, width/2 - r, height/2 - r)` auf. Hintergrund: Der Constructor setzt den Body-Circle einmalig anhand der **Default-Thorn-Textur (36×22)**, aber der Pool recycelt Sprites über Texturen unterschiedlicher Dimension — **Thorn / SapphireThorn = 36×22**, aber **BloodProjectile / MansionMissile / FlameMissile = 16×16**. Phasers `setCircle`-Offset ist frame-relativ; sobald in `fire()` eine 16×16-Textur gesetzt wurde, blieb der Offset auf `(18-r, 11-r)` stehen → der Body driftete ~10px nach rechts und ~3px nach unten weg vom sichtbaren Bullet (User-flagged: "die Hitbox sitzt davor", sichtbar mit F2-Overlay beim Marquis). Sichtbar war's nur auf Onyx (Marquis-Fan/Homing, Cursed Mirror, Candelabra, Prismarch-Specials) + Marquis — Emerald/Sapphire-Mobs nutzen die 36×22-Thorn-Silhouette und waren zufällig korrekt. **Universal:** alle Gegner/Bosse feuern durch den einen geteilten `enemyProjectilePool`, keiner ruft `setTexture` direkt auf → der Recenter gilt automatisch konsistent für jedes Projektil. Player-Pfad (MagicMissile) ist davon unberührt: Spellblade recentert via `bodyRadiusOverride`-Branch, Wizard bleibt auf der Constructor-Textur.

**Missile-Modifier-System (Phase 6 — 2026-05-07):** Vier PlayerStats-Felder steuern die Missile-Modifier, datengetrieben über reguläre `ItemEffect`s:
...
- **`homingTurnRate`** (base 0 deg/s): max Turnrate pro Frame. **Kein Player-Item granted das mehr** seit dem Wizard-Glasses-Rework 2026-05-09 (siehe unten) — die Stat ist aber noch im Code aktiv damit `EnemyProjectile.setHoming()` für Cursed Mirror, Lord Onyx Phase 3 + Forest Wrath, und Marquis-Mirror-Special weiterhin funktioniert. Wenn ein zukünftiges Item homingTurnRate granted, kicked die Player-Side-Mechanik in `MagicMissile.tickHoming` automatisch wieder ein.
...
- **`multishotCount`** (base 1, neu 2026-05-09): wie viele parallele Bolts pro Cast. Wizard Glasses setzt das auf 2. `Player.fireWizardMissile` + `fireSpellbladeBolt` lesen den Stat, looping einen `computeMultishotOffsets(dir, count)` Helper für die perpendikulären Spawn-Offsets (`MULTISHOT_OFFSET_PX = 14` px Gap), und scaled per-Shot-Damage auf `MULTISHOT_DAMAGE_MULT = 0.80` sobald count > 1. Beide Charaktere routen durch denselben Pool-`fire()` mit charakter-spezifischen Bolt-Options (Texture, Pierce-Baseline, Velocity-Inheritance, bodyRadiusOverride) — der Multishot-Loop ist character-agnostic. **Wizard Glasses Rework**: war ursprünglich `homingTurnRate +80°/s + missileSpeed +10%`, geswapt nach User-Feedback "homing macht keinen spass, ich skippe das item eigentlich immer. und beim neuen char funktioniert es auch nicht gut" (Spellblade-Bolt rotiert visuell auf seine Velocity → mit Homing kurvt der Diamond-Sprite weird durch die Bahn). Neue Effekte: `multishotCount +1 + missileSpeed +10%`. Damage-Trade-off explicit: 2 Bolts × 0.80 = **1.6× DPS** auf großen Hitboxen (Bosse), aber 0.80× DPS wenn Trash-Mob nur einen Bolt eats — Sniper-Trade-off, vermeidet generic +damage-Item. **Tuning-Verlauf:** initial 0.75 (1.5× / 0.75×), gebumped auf 0.80 nach User-Feedback "fühlt sich kaum wie ein upgrade an" weil der Trash-Single-Hit-Case zu schmerzhaft war. 0.80 hält den Boss-Killer-Charakter (Item ist nicht für Group-Clearing) liest sich aber spürbarer als Upgrade.
...
**Daten-Modell:**
- `ActiveItemKind` discriminator (`'echoesOfBlood'` + `'transmuteCoinsToKey'` + `'sacrificialStatUp'` seit 2026-06-12). Neue Active = ein Eintrag hier + ein switch-arm in `GameScene.tryActivateActiveItem` + ein `active: { kind: '...' }` Field auf einer ItemDefinition.
- `ActiveItemSpec`-Interface trägt kind-spezifische Params (z.B. `bossDamageFraction` für Echoes, `coinCost` für den Transmutation Stone).
...
**`ItemDefinition.metaUnlock`:** Optional. Wenn gesetzt (= stable enemy id wie `'boss-marquis-of-mirages'`), filtert `pickItemFromPool` das Item aus dem Pool aus solange `MetaProgress.hasBeatenBoss(metaUnlock) === false` ist. Nach erstem Marquis-Kill in irgendeinem Run wird Blood of Marquis ab dem nächsten Run in **Treasure-Räumen auf jedem Floor** sichtbar (war bis 2026-05-09 Onyx-Boss-Pool, dann gemoved weil Late-Onyx-only-Drop praktisch sinnfrei war — User-Bug "man hat's nur noch für den Prismarch und muss Marquis 2× across runs schlagen um's zu sehen"). Existing items (Bloodbound Chalice etc.) haben kein metaUnlock → sind von Anfang an verfügbar.

**Active-Item-Swap (Isaac-Style, 2026-06-12):** Ein zweites Active aufheben **ersetzt nicht mehr stillschweigend** das erste — das alte droppt als physisches Pedestal (User-Bug: "das erste ist weg, man sollte tauschen können"). Zwei Hälften:
- **Bookkeeping (`ItemSystem.unpickEquippedActiveFor`)**: rollt beim Swap das alte Active komplett zurück — `StatsSystem.removeModifiersByItemId` (sonst Stack-Exploit: Marquis aufheben → wegtauschen → re-pick = +30% doppelt), `PlayerHealth.clearMaxHealthCap()` (Cap-Lift, verlorene HP-Ups während des Caps werden NICHT nachgewährt), `removeMaxHealth(maxHealthBonus)` falls gesetzt, `picked.delete(id)` (Floor-Uniqueness + Re-Collect funktionieren). Drop-Profile-Felder (suppressHeartDrops/coinDropMult) werden bewusst nicht revertiert (one-way Aggregates, dormant). Pact-Stat-Ups bleiben (runtime-granted, schon bezahlt).
- **Physischer Drop (`GameScene.spawnDroppedActive`)**: `tryCollectPickup` snapshottet das equipped Active vor/nach `onCollect`; bei Wechsel spawnt das alte als `ItemPickup` **1 Tile vom Player weg** (Player→Pedestal-Richtung verlängert, bounds-clamped) mit 700ms Spawn-Protection — direkt unterm Player würde der Auto-Collect die beiden Actives endlos ping-pongen. **`ItemPickup.droppedActive`-Flag + `PickupSnapshot.droppedActive`**: dropped Actives sind die EINZIGE Item-Kind die in JEDEM Raum durch `pendingPickups` round-trippt (Flag wird beim Restore re-attached, sonst überlebt das Pedestal nur einen Leave/Return-Zyklus).
...
- Equipped + usable → full color, Trim-Ring 0.85 Alpha
- Equipped + un-usable → Icon tinted `0x404048` + alpha 0.55, Label alpha 0.4, Trim 0.35 Alpha
- `activateItem:activated` event triggert kurzes Trim-Ring-Punch-Up (4-px Stroke + alpha-tween, 280ms)

**Usability ist per-kind** (2026-06-12): `echoesOfBlood` → HP ≥ 2, `transmuteCoinsToKey` → Coins ≥ `coinCost`, `sacrificialStatUp` → max HP ≥ 4 (mindestens 2 Herz-Container, einer muss das Opfer überleben; Slot tracked dafür zusätzlich `currentMaxHp` aus `player:healthChanged`). Slot listened auf `activeItem:equipped` (rebuild Icon), `player:healthChanged` (HP-Gate-Refresh), `inventory:changed` (Coin-Gate-Refresh), `activeItem:activated` (flash). Constructor primed from `registry.get('activeItemSystem')` + `registry.get('playerHealth')` + `registry.get('inventory')` damit floor-transition-rebuilds sofort den korrekten State zeigen ohne auf erste Hit-/Equip-/Coin-Event zu warten.
...
- **Cost-Gate per-kind im switch** (war bis 2026-06-12 universal HP-gate): `echoesOfBlood` braucht HP ≥ 2 (Activation kostet HP), `transmuteCoinsToKey` zahlt via `inventory.spendCoins(coinCost)` — atomic, failed spend = kein Effekt + keine Activation-Event
- Switch on `equipped.kind` → execute-method
- Bei Erfolg: `EventBus.emit('activeItem:activated', { itemId })`

**Transmutation Stone (zwei Modi, auto-upgrade by wallet — User-Decision 2026-06-12):** [Q] konvertiert **5 Coins → 1 Key** ODER **30 Coins → 1 Random-Treasure-Item** (war 20, gebumped mit dem Shop-Price-Pass), automatisch: ab `itemCost` (30) Coins feuert die Item-Konversion, darunter die Key-Konversion. Trade-off der Auto-Variante (bewusst akzeptiert): mit vollem Geldbeutel macht [Q] keine Keys mehr — Keys bleiben aber im Shop für 8 kaufbar. **HUD-Mitigation:** der [Q]-Slot-Label zeigt `[Q] KEY` / `[Q] ITEM` je nach Kontostand (via `inventory:changed`), der Threshold-Switch überrascht nie. Item-Roll (`executeTransmuteCoinsToItem`): seeded RNG `${dungeonSeed}-transmute-${rollCount}` (per-run-Counter, SHUTDOWN-reset), Treasure-Pool via `pickItemFromPool` mit **exclude = pickedIds ∪ getFloorReservedItemIds()** (gleiche Union wie `spawnBossPoolItem` — kein Duplikat mit Shop-/Treasure-/Boss-Slots floor-weit). Spawnt als Pedestal 1 Tile über dem Player (normaler Collect-Flow → Toast + SFX + Uniqueness-Bookkeeping). Pool exhausted → graceful fallback auf Key-Konversion. Coins werden nur bei Erfolg gespent (atomic). 30 Coins random ist bewusst ein SCHLECHTERER Deal als ein Shop-Item (12-22, gewählt) — das Premium kauft Verfügbarkeit überall. (Die frühere Pact-Synergie-Notiz ist mit dem Pact-V2-Rework obsolet — der Pact ist kein Coin-Mult mehr.) Visual: Gold-Ring am Player (Key = 1 Ring, Item = Doppel-Ring größer+heller) + `playDoorUnlock()`-SFX.
...
- Pool-Placement: Blood of Marquis sitzt im **Treasure-Pool** (floor-agnostic), `floor`-Tag entfernt. Pool-Move 2026-05-09 weil das Item vorher Onyx-Boss-Pool-locked war → effektiv nur ein Drop-Spot pro Run + Marquis muss 2× across-runs geschlagen werden um's überhaupt zu sehen → praktisch sinnfrei. Treasure-Pool macht's zum mid-run pivot pickup, der `metaUnlock`-Gate (Marquis once beaten) bleibt
...
## Elite-Räume, Minibosse & Floor-Länge (2026-06-12)

**Anlass:** User-Feedback "Run dauert ~10 Minuten, recht kurz" — aber nicht einfach mehr gleiche Räume, sondern mehr Räume + neue Begegnungs-Tiers. Risiko-Treppe: Mob-Raum < Elite < Miniboss < Boss.

**Raumanzahl:** `FloorTheme.roomCount` — Emerald 10 / Sapphire 12 / Onyx 14 (war flat `DUNGEON_TARGET_ROOM_COUNT = 8`, bleibt als Fallback). 5×5-Grid hat mit 25 Zellen genug Headroom; Regression-Test deckt 10/12/14 ab.

**Generator-Tagging** (`GenerateOptions.eliteRoomCount` + `minibossChance`): Läuft NACH der Special-Room-Platzierung — nur plain-Normal-Räume sind Kandidaten (nie Start/Boss/Treasure/Shop, per Test über 100 Seeds abgesichert). Elite-Räume kriegen `RoomDescriptor.elite = true` + `enemySpawnCount = 1`; der Miniboss-Roll (`MINIBOSS_SPAWN_CHANCE = 0.35`, **wird unconditionally gerollt** damit die RNG-Sequenz unabhängig von der Kandidaten-Lage stabil bleibt) taggt max. EINEN Raum mit `miniboss = true` + `enemySpawnCount = 0`. **Türen bleiben Normal-Texturen** — Elite/Miniboss-Räume sind absichtlich Überraschungen (Isaac-Style), keine markierten Specials. GameScene gated den Chance-Roll auf `theme.minibossIds.length > 0` (Floor ohne Kandidaten rollt nie).

**Champion-System (`BaseEnemy.promoteToElite(pool)`):** Elite-Raum spawnt EINEN gewichtet gepickten Floor-Mob (Species stable per seed+room) und promotet ihn: HP ×**`FloorTheme.eliteHpMult`** (Emerald/Sapphire **6**, Onyx **5.1** — Onyx-Mobs sind hoch authored UND tragen ×2.0 Floor-Mult, die volle ×6 stapelte Champions fetter als den Miniboss; Tuning-Verlauf ×4.5 (−25%) war im Playtest zu weich → ×5.1 (−15%) User-Pick; Registry-Slot `eliteHpMultiplier`, Fallback `ELITE_HP_MULT`, ON TOP des Floor-Mob-Multiplikators), Scale ×1.5 (Hitbox wächst mit — bewusst, größeres Target), **knockback-IMMUN** (via `knockbackFactor: 0` in der promote-time Definition-Copy — ein solo Champion der per Dauerbeschuss in die Ecke geschoben werden kann ist kein Encounter; gleiche Logik wie Boss-Knockback-Ignore), pulsierende **rote Aura** (`ELITE_AURA_COLOR = 0xe02838` — war gold, geswapt im Aggression-Pass: rot liest als Gefahr, gold bleibt für Boss/Miniboss-HUD-Framing + Coin-Payouts reserviert; loses Scene-Object, Cleanup in `die()` UND `destroy()` — Bloomheart-Pattern), und ein **universeller Radial-Burst** (random Basis-Rotation, feuert auch durch Knockback — sonst wäre ein Champion durch Dauerbeschuss stumm-lockbar). Der Burst ist der Kern-Trick: er macht JEDEN Roster-Pick gefährlich, auch reine Melee-Chaser. **Aggression-Pass (2026-06-12, User-Anlass "Champion spawnt immer solo → kann wesentlich aggressiver sein"** — der Elite-Branch in `spawnEnemiesForRoom` early-returnt vor dem Pack-Fill, ein Champion trägt das volle Raum-Threat-Budget allein): (a) Burst-Intervall **3.0s → 2.2s** (`ELITE_BURST_INTERVAL_MS`), (b) **Dual-Wave-Burst** Gungeon-Style à la Bog Colossus — zweite 6-Thorn-Welle `ELITE_BURST_SECOND_WAVE_DELAY_MS = 350ms` später, um exakt einen Half-Step (30°) rotiert, sodass die Safe-Lanes von Welle 1 die Thorn-Bahnen von Welle 2 sind (Lane-Camping unmöglich, Player muss sich zwischen den Wellen bewegen; deferred Callback mit `!active || !scene || hp <= 0`-Guard — Bloomheart-Freeze-Pattern), (c) **Move-Speed ×1.3** (`ELITE_MOVE_SPEED_MULT`) via per-Instanz-Definition-Copy in `promoteToElite` — `BaseEnemy.definition` ist dafür nicht mehr `readonly` (alle Subclass-AIs lesen `this.definition.moveSpeed` pro Frame, die Copy bufft sie alle ohne tickAI-Edits; das shared `ENEMIES`-Entry wird NIE mutiert, nur die Referenz durch eine Spread-Copy ersetzt; rooted Mobs mit moveSpeed 0 bleiben rooted). Champion-Tod: garantierter 3-Coin-Burst zusätzlich zum normalen Roll — **das ist der komplette Elite-Payout**: der frühere zusätzliche Raum-Clear-Pickup (`spawnEliteClearReward`, seeded Heart/Key/Coin-Roll) wurde 2026-06-12 entfernt, weil Champion-Drop + Raum-Drop als Double-Dipping lasen (User: "da reicht der Champion-Drop völlig aus"). **Scale-Trap (Bug-Fix 2026-06-12):** Mob-Klassen mit Scale-Animationen MÜSSEN relativ zur aktuellen Scale tweenen/resetten — Bog Frogs Cheek-Puff-Telegraph tweente auf absolute Werte (`scaleX: 1.12`) und resettete mit `setScale(1, 1)`, was JEDEN Frosch nach dem ersten Schuss auf Scale 1.0 schrumpfte (statt `WORLD_SPRITE_SCALE` 1.25) und die Champion-×1.5 komplett nukte (User: "Champions sind so klein wie normale Mobs, normale Mobs shrinken nach kurz"). Fix: `baseScaleX/Y` lazy beim ersten Telegraph gecaptured (promoteToElite läuft synchron nach Construction, also vor dem ersten Tick), Tween-Targets + Reset multiplizieren damit. **Zweite Instanz am selben Tag: Snapper Bloom** — Open-Mouth-Telegraph (`scaleX: 1.18`) + Snap-Shut-Reset (`scaleX: 1`) hatten denselben Absolut-Wert-Bug; gleicher baseScale-Fix. (Beim ersten Sweep übersehen weil der Grep nur `setScale|scale:` suchte — Tween-Properties heißen `scaleX:`/`scaleY:`.) Wer einen neuen Mob mit Squash/Stretch baut: nie absolute Scale-Werte, immer Basis capturen.

**Minibosse** (`FloorTheme.minibossIds`, je 1 Kandidat pro Floor; HP authored gegen den Floor-Multiplikator → effektiv ~28/39/54, **PLUS Boss-DPS-Scaling on top** — Reversal 2026-06-12, war ursprünglich exempt ("gestackter Build darf shredden"), aber im Playtest starb einer near-instant und der User wollte das Boss-Prinzip "fühlt sich immer wie base stats an" auch hier. Implementation: `BaseEnemy.constructor` liest `bossHpScale` für `miniboss: true`-Entries und rechnet `definition.hp × mobHpMult × dpsScale`; alle 3 Spawn-Pfade (`spawnMinibossForRoom`, `devSpawnMiniboss`, `__wiz.spawnMiniboss`) rufen vorher `updateBossHpScale()`. Die Silber-HP-Bar snapshottet maxHp NACH dem Scaling): Reguläre `BaseEnemy`-Subklassen über die normale Factory → leben in der `enemies`-Group, Room-Clear/Türen/Kill-Flow brauchen NULL Sonderbehandlung. Nur Spawn-Entry (`spawnMinibossForRoom`: Raum-Center, vom Player weggeschoben) und Clear-Reward (`spawnMinibossClearReward`) sind eigene Pfade. **Clear-Reward (umgebaut 2026-06-12, garantiertes Item war User-flagged "zu viel"):** seeded Roll `${dungeonSeed}-miniboss-reward-${roomId}` — `MINIBOSS_ITEM_DROP_CHANCE = 0.25` auf Treasure-Pool-Item-Pedestal (Jackpot-Moment, via `spawnTreasureItemAt(x, y, desc)` mit `treasureItemId`-Snapshot + Floor-weiter Item-Uniqueness), sonst **Pickup-Paket** (Heart links + Key rechts + `MINIBOSS_REWARD_COIN_COUNT = 3` Coin-Spray drunter, alle mit 700ms Spawn-Protection) — zahlt sichtbar fetter als der Elite-Single-Pickup. Uncollected Payouts überleben Leave/Re-Entry: Pedestal via `captureBossRewards` (deckt `desc.miniboss` mit ab), Pickups via `pendingPickups`. **Wichtig:** der "leerer Raum → sofort cleared"-Guard in `enterRoom` excludet `desc.miniboss` (sonst Türen sofort auf).

**Miniboss-HP-Leiste (2026-06-12):** `src/ui/MinibossHealthBar.ts` — kleine silberne Top-Bar als "kleine Schwester" der Boss-Bar: **200px** breit (Boss: 400), **Silber-Frame** (`0xb8c4d0`, Boss: gold), Name 13px UPPERCASE in silber (Boss: 18px gold), `ANCHOR_Y = 64` (unter der Boss-Bar-Zone, kein Overlap selbst wenn Dev-Spawn beide gleichzeitig zeigt). Eigene Event-Familie `miniboss:spawned { name, maxHp }` / `miniboss:hpChanged { current, max }` / `miniboss:gone` (void — deckt Kill UND Despawn-ohne-Tod ab, z.B. Dev-Menu-Room-Clear). Emissionen leben in **BaseEnemy**, gated auf neuen `EnemyDefinition.miniboss?: boolean`-Tag (gesetzt auf den 3 Miniboss-Entries): Constructor → spawned (maxHp NACH Floor-Mob-Mult snapshotted), `takeDamage` + Burn-Tick → hpChanged, `die()` + `destroy()` → gone (mit `minibossGoneEmitted`-Flag gegen den die→tween→destroy-Doppelpfad). UIScene instanziiert das Widget direkt neben `BossHealthBar`.

- **Thornwood Shambler** (Emerald, HP 28): langsamer Dauer-Walker (45 px/s), aimed 3-Thorn-Fan alle 2.2s, jede 3. Volley stattdessen telegraphierter 8-Thorn-Radial (420ms Tint-Flash + enemy:charge).
- **Mire Lurker** (Sapphire, HP 26): taucht ab (intangible via `body.checkCollision.none`, Wraith-Pattern; gleitet mit 130 px/s zum Player), taucht mit 450ms-Telegraph + 6-Thorn-Radial auf, surfaced 2.4s mit aimed Shots alle 800ms, dann wieder runter. Rhythmus = "punish the surfaced window".
- **Doppelgänger** (Onyx, HP 30 — **ersetzte den Headless Knight 2026-06-12** auf User-Wunsch "ein Mimic, ein Wizard der aussieht wie der Spieler"): Dunkles Spiegelbild des Spielers. **Texture = `drawWizardTexture` mit eigener Onyx-Palette** (`DOPPELGANGER_WIZARD_PALETTE`: schwarz-violette Robe, aschfahle Haut, schattiger Bart, Mansion-Gold-Schnalle, **glühende Amethyst-Augen** als Tell) — Änderungen an der Spieler-Silhouette updaten den Mimic automatisch mit. AI: **kited wie ein Spieler** (Distanz 200 px ± 35 Deadband, perpendikulärer Strafe mit jittered Richtungswechsel) und feuert dabei **gerade Filler-Casts alle 1.1s** (MansionMissile, kein Homing — der Mimic schießt zurück wie du, keine passiven Fenster). Alle 3.8s die große Sequenz: **Blink auf Flanken-Position** (Fade-out 170ms → Player→Mimic-Vektor um ±70-110° rotiert @ Kite-Distanz, bounds-clamped → Fade-in) → Telegraph (500ms, Amethyst-Tint + charge-Cue, rooted) → **3 sequentielle Homing-Missiles** à la Marquis-Mirror-Special (320ms Spacing, 110°/s Turn-Rate, 2200ms Lifetime), **gefächert ±14°** damit sie aus verschiedenen Vektoren konvergieren — ein einzelner Cut dodgt nicht mehr alle drei. **Soft-Enrage < 50% HP** (kein Phasen-System): Volley-Intervall 3.8s → 2.8s + Strafe-Speed ×1.3. Difficulty-Pass 2026-06-12 nach User-Flag "zu einfach" (V1 war zwischen Volleys komplett passiv; Filler + Blink + Spread + Enrage + HP 27→30) — **finales Tuning user-validated: "fühlt sich sau geil an"**. Knight-Code komplett entfernt (Klasse, KNIGHT_*-Konstanten, Texture, Factory-Case). **Zwei Playtest-Fixes (2026-06-12 spät):** (a) **Blink-Safe-Zone** — der rohe Bounds-Clamp konnte den Flank-Spot bei wall-hugging Player direkt auf ihn ziehen ("hat sich einige Male direkt auf mich drauf gespawnt"); `pickBlinkSpot` probiert jetzt 5 Kandidaten-Rotationen (Flank → Mirror-Flank → ±135° → 180°) und nimmt den ersten dessen GECLAMPTE Position ≥ `SAFE_SPAWN_DISTANCE` (3 Tiles) vom Player hält, sonst best-effort den weitesten (Pixie-Queen-Teleport-Philosophie). (b) **Sichtbarkeits-Pass** — die fast-schwarze Robe verschwand auf dem dunklen Mansion-Parkett; `DOPPELGANGER_WIZARD_PALETTE` auf leuchtendes Mid-Amethyst gelifted (Robe 0x6a3aa4, Hi 0x9a62e0 — weiter klar der "dunkle Spiegel" in kaltem Violett) plus **Amethyst-Boden-Glow-Ellipse** die dem Sprite folgt und seine Alpha spiegelt (Blink-Fade nimmt den Glow mit — kein "Glow verrät die unsichtbare Position"-Leak; loses Scene-Object, Cleanup in destroy()).

Alle drei Texturen prozedural in PreloadScene (64×64, deutlich größere Silhouetten als Mobs, floor-palette-treu). Alle deferred Callbacks mit `!this.active || !this.scene`-Guards (Freeze-Pattern). `coinDropChance: 1` (garantierter Coin on top des Clear-Rewards).

**Dev-Hooks:** `__wiz.spawnElite()` (random Champion im aktuellen Raum), `__wiz.spawnMiniboss(id?)` (Floor-Default oder explizite Id). Das **[B]-Boss-Spawn-Menü** listet die drei Minibosse zusätzlich in ihren Floor-Sections ("· MINI"-Suffix, dimmer) — Klick spawnt via `devSpawnMiniboss` (regulärer Factory-Pfad, KEINE Boss-Bar / No-Hit-Tracking, im Gegensatz zu den Boss-Rows).

**Tests:** `tests/EliteMiniboss.test.ts` (7 Cases) — Elite-Count + Kind-Invariante, Miniboss-Exklusivität (nie elite+miniboss gleichzeitig), Chance-0/1-Verhalten, Special-Room-Guard über 100 Seeds, Determinismus, statistische Bounds der 0.35-Chance, 10/12/14-Raumanzahl.

**Offen (Chunk 3):** Neue dedizierte Elite-Gegner ZUSÄTZLICH zu den Champion-Varianten (User wollte explizit beides — Champions sind die Foundation, eigene Elite-Designs der Content-Pass oben drauf). Außerdem offen: Minimap-Markierung für gecleared Elite-/Miniboss-Räume (bewusst v1 weggelassen).

---

## Visual / UX Polish

### Art-Style: Flat-Vector-Overhaul (2026-06-13)

Das gesamte Spiel wurde vom ursprünglichen Pixel-/painterly-Look auf einen **Flat-Vector / Cartoon-Stil** ("B"-Richtung) umgezeichnet — vom User validiert. **Bleibt prozedural** (Phaser `Graphics`/`generateTexture` in `PreloadScene.ts`), KEIN Wechsel zu PNG/Aseprite-Asset-Pipeline. (Der Tech-Stack-Hinweis oben auf "PNG/Aseprite bevorzugt" ist damit obsolet — Texturen sind weiterhin alle code-gezeichnet.)

**Stil-Rezept (konsistent über alle Texturen):**
- **Kräftige dunkle Outline** pro Form — entweder Form in OUTLINE-Ton + dieselbe Form ~1.5–2 px inset im Fill-Ton, ODER `lineStyle(2–2.5, OUT)` + `strokePoints`/`strokeEllipse`/`strokeCircle`.
- **Flache Single-Tone-Fills** — keine Gradient-Bänder / kein Dithering. Palette-Hue-Identität wahren (Emerald grün, Sapphire blau/cyan, Onyx purple-black/gold/amethyst, Crimson rot).
- **Genau ein weiches Highlight** pro Hauptform (helle Ellipse/Arc oben-links).
- **Runde, glatte Silhouetten**; Original-Silhouette + Signature-Features jeder Figur erhalten.
- **Cute Augen** = dunkler Punkt + winziger weißer Catch-Light-Punkt.

**Wichtige Regeln / gelernte Lektionen:**
- **Sizing:** Gezeichnete Sprite-Größe muss nah an der Hitbox bleiben — Texturdimensionen + Entity-Scale NIE ändern (Hitbox hängt dran), nur die gezeichnete Form im Frame. (Bug: Slimes wurden zu groß gezeichnet → Mossy-Slime-Dome 36→30 px, Mossy-Behemoth-Dome 60→48 px getrimmt.)
- **Vorab-vektorartige Sprites** (z.B. handgemalte Minibosse Grovekeeper/Bog Hag, oder die mehrtonigen Sapphire/Onyx-Mobs) werden von Bulk-Konvertierungen gern als "schon konform" übersprungen → fühlen sich "nicht angefasst" an. Lösung: expliziter **Bolder-Pass** (dicke Outline-Strokes + sattere Flächen), nicht überspringen. Erledigt für: Grovekeeper, Bog Hag, Bog Frog, Damselfly, Bog Tortoise, Possessed Candelabra, Cursed Mirror.
- **Köpfe bei winziger Skala:** zu viele dunkle Mini-Features (Narbe + dunkles Auge + Lippen + Fang) matschen zu einem "dunklen Bart-Band". **Marquis of Mirages** Kopf neu: zurückgekämmte Haare ON TOP (hohe Witwenspitze + Koteletten-Rahmen, nie ins Gesicht), blasses kantiges Gesicht, **zwei klare rote Glüh-Augen** (statt 1 rot + 1 dunkel + Narbe), spitzes Ohr, dünner Mund + ein Fang. **Pixie Queen** entgrimmt: gerader Strich-Mund → freundliches U-Lächeln + rosige Wangen + größere Augen.
- **MainMenu-Title-Art** rendert Wizard + Pixie Queen als reine `setScale(4)`-Images der `TextureKeys.Player` / `TextureKeys.BossPixieQueen` — Textur-Änderungen wirken dort **automatisch**, kein separates Gesicht.

**RoomAtmosphere:** Der forest-Pfad (Wand-Bänder + Boden-Patches) war als einziger painterly (3-Ton-Laub-Blobs) und wurde geflattet (`paintForestTopBand`/`BottomBand`/`SideBands` + `paintMossyPatches`). Swamp + Mansion waren schon flach-geometrisch (Backstein/Stein/Samt + Gold-Leisten) und blieben.

**Mockups:** `mockup.html` + `src/scenes/StyleMockupScene.ts` (+ Wiring in `main.ts`/`GameConfig.ts`-SceneKey/MainMenu-`[K]`-Bind) wurden **gelöscht** — Mockups laufen jetzt über den Brainstorming-Visual-Companion (Browser). Dead-Code-Reste: ungenutzte `px`/`pxBlock`-Pixel-Helfer + die verwaisten `drawBossCrimsonLordTexture`/`drawBossSapphireMarquisTexture` in `PreloadScene` (kein Consumer mehr) — Cleanup-Kandidaten.
...
**`[U] BETA: UNLOCK ALL`-Hint** (`setupBetaUnlockHint`, **TEMPORÄR — vor Production-Release entfernen**): top-right Amber-Hint (`#ffb84a` damit's visuell aus dem normalen weißen [I]/[S]/[M]-Hint-Family rausfällt und als debug-affordance liest). Auf [U]-Press → `MetaProgress.unlockAllForTesting(Object.keys(ITEMS))` flippt drei Gates synchron: (a) `'boss-lord-onyx'` in `bossesDefeated` UND `bossesDefeatedAsSpellblade` → unlocked Spellblade-Charakter + Wizard-Prismancy-Skin + Spellblade-Prismarch-Skin auf einen Schlag, (b) `'boss-marquis-of-mirages'` in `bossesDefeated` → entriegelt Blood of Marquis (metaUnlock-Gate), (c) alle Item-Ids in `itemsDiscovered` → Stats-Trophy-Overlay zeigt vollen Collection-Status. Nach `unlockAllForTesting` zeigt's ein 36px gold "EVERYTHING UNLOCKED"-Flash zentral (220ms ScaleIn + 600ms Hold + 600ms FadeOut) und ruft `scene.restart()` damit Char-Cycle + Skin-Toggle live re-paintet werden. **Cleanup für Production:** zwei Stellen löschen — `MetaProgress.unlockAllForTesting()` Method + `MainMenuScene.setupBetaUnlockHint()` Method + den Aufruf in `create()`. Beide haben "Remove this when the beta closes"-Marker im JSDoc.
