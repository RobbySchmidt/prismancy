...
- [x] **Berserker-Trigger** = Body unter **25 %** seiner HP via `VAMPIRE_BERSERKER_HP_FRACTION = 0.25` (war 0.30). Tracked per body in `VampireBody.takeDamage`. Mit dem Shared-Pool-Gate (siehe nächste Bullet) kann der Marquis effektiv nur dann berserker werden, wenn der Lord schon tot ist — solange der Lord lebt, blockt der Coordinator alle Damage auf Marquis, also bleibt seine HP bei `maxHp` und der Threshold wird nie erreicht.
- [x] **Shared-HP-Pool-Gate** (User-Wunsch: "die teilen sich einen HP pool. Lord geht in Phase 2, Mage kann erst in Phase 2 wenn Lord tot"): `VampireFightCoordinator.shouldBlockDamage(body)` returnt true wenn `body instanceof SapphireMarquis && CrimsonLord aktiv`. `VampireBody.takeDamage` checked das vor jedem Hit; bei block: gold tint flash (`flashShielded`, gleicher Visual-Idiom wie Prismarch-Invuln), keine HP-Änderung, keine Coordinator-Events. Effektiv: Damage geht durch den Lord first (drains erste Hälfte der combined HP bar), dann nimmt Marquis Damage (zweite Hälfte). Player-Feedback: bei Treffern auf den geschilden Marquis blitzt er kurz golden auf.
