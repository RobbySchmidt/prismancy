...
- [x] **Death-Pfad**: `boss:killed` payload checks `name === 'The Prismarch'` → bypasses normal reward flow → calls `handleLordOnyxKilled` → `Cosmetics.unlockPrismancySkin()` + Camera-Flash + Shake + emit `run:onyxFullVictory` + 900ms Mini-Celebration-Window → `transitionToEndScene('full')` → fade-to-black → `EndScene` übernimmt die "VICTORY"-Beat. **Der vorherige in-room VICTORY-Banner + "Prismancy Skin Unlocked"-Toast sind entfernt** (User-Feedback: doppelte Anzeige während der Camera-Fade hat den EndScene-Headline geclasht). Skin-Unlock recording bleibt unverändert; das EndScene zeigt den eigentlichen "VICTORY" + subtitle dann auf eigenem Screen.
